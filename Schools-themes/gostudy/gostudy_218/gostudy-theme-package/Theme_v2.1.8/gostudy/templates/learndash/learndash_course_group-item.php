...
$learndash_completed_text = Gostudy_Theme_Helper::get_mb_option('learndash_completed_text');
 ...
 if (class_exists('SFWD_LMS')) {
   ...
   if ( ! is_null( $options ) ) {
      if ( isset($options['modules'] ) && isset( $options['modules']['groups_options'] ) && isset( $options['modules']['groups_options']['groups_paypal_currency'] ) )
         $currency = $options['modules']['groups_options']['groups_paypal_currency'];
   ...
   $price = $course_options && isset($course_options['groups_course_price']) ? $course_options['groups_course_price'] : $learndash_free_text;
   ...
   if ( $has_access && ! $is_completed ) {
      ...
      $class = ! empty( $course_options['groups_course_price'] ) ? 'ld_course_grid_price price_' . $currency : 'ld_course_grid_price free';
      $ribbon_text = $price;
   }
?>
...
<?php 
  // For legacy_short_description
	...
	if ( $post_type == 'groups' ) {
	    $legacy_short_description = isset( $course_options['groups_course_short_description'] ) ? $course_options['groups_course_short_description'] : '';
	}
