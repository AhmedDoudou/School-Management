...
if (!class_exists('Gostudy_Get_Page_Title')) {
    ...
    class Gostudy_Get_Page_Title
    {
        ...
        {
            if(class_exists('SFWD_LMS') && is_singular( 'sfwd-courses' )) : // Check if LearnDash header
   ...
   //                      $title = apply_filters('gostudy/page_title/courses', $title);
            ...
            elseif (is_archive()) {
                ...
                elseif (class_exists('SFWD_LMS') && is_post_type_archive('sfwd-courses')) {
                    $title = Gostudy::get_option('ld_courses_archive_page_title');
                } 
